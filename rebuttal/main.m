    ...
    for gp = gp_list
        ...
        while iter < num_iter
            ...
            for delta = sort([delta_list, delta_bound])
                [supp_est1, supp_est2] = GPM(M, delta);
                exact_recovery1 = double(sum(supp_est1==true_idx)==d);
                gpm1_res = [gpm1_res, exact_recovery1];
                exact_recovery2 = double(sum(supp_est2==true_idx)==d);
                gpm2_res = [gpm2_res, exact_recovery2];
                if delta == delta_bound
                    selected_gpm1_res = exact_recovery1;
                    selected_gpm2_res = exact_recovery2;
                end
            end
            ...
            for delta = sort([delta_list, delta_bound])
                [supp_est1, supp_est2] = GPM(M_complete, delta);
                exact_recovery1 = double(sum(supp_est1==true_idx)==d);
                gpm1_complete_res = [gpm1_complete_res, exact_recovery1];
                exact_recovery2 = double(sum(supp_est2==true_idx)==d);
                gpm2_complete_res = [gpm2_complete_res, exact_recovery2];
                if delta == delta_bound
                    selected_gpm1_complete_res = exact_recovery1;
                    selected_gpm2_complete_res = exact_recovery2;
                end
            end
            ...
            results = [graph_properties, selected_sdp_res, selected_sdp_complete_res, sdp_res, ...
                ...
                gpm1_res, gpm2_res, gpm1_complete_res, gpm2_complete_res, ...
                selected_gpm1_res, selected_gpm2_res, selected_gpm1_complete_res, selected_gpm2_complete_res];
