...
xlabel({'$\frac{\psi(\mathcal{G}_{J,J})}{\phi(\mathcal{G}_{J,J})}$'}, 'FontSize', 30,'Interpreter','latex'); 
ylabel('Exact Recovery Rate', 'FontSize', 16); 
title({'Complete outside J x J',''}, 'position', [9, 0.9], 'FontSize', 13);
legend({'$\bar{\lambda}(\textbf{\textit{M}}^*)=1$', ...
    '$\bar{\lambda}(\textbf{\textit{M}}^*)=2$', ...
    '$\bar{\lambda}(\textbf{\textit{M}}^*)=5$', ...
    '$\bar{\lambda}(\textbf{\textit{M}}^*)=10$'},...
...
i = 1;
for spec_gap = [1 2 5 10]
    res = readmatrix(strcat(['../simulation1_noisy_dense/results/s', int2str(s), '_spec_gap', int2str(spec_gap), '.csv']));
...
xlabel({'$\frac{\psi(\mathcal{G}_{J,J})}{\phi(\mathcal{G}_{J,J})}$'}, 'FontSize', 30,'Interpreter','latex'); 
ylabel('Exact Recovery Rate', 'FontSize', 16); 
title({'Complete outside J x J, with noise',''}, 'position', [9, 0.9], 'FontSize', 13);
legend({'$\bar{\lambda}(\textbf{\textit{M}}^*)=1$', ...
    '$\bar{\lambda}(\textbf{\textit{M}}^*)=2$', ...
    '$\bar{\lambda}(\textbf{\textit{M}}^*)=5$', ...
    '$\bar{\lambda}(\textbf{\textit{M}}^*)=10$'},...
...
f = gcf;
set(f, 'Position',  [100, 100, 1050, 320])

exportgraphics(f,strcat('simulation1_dense.png'),'Resolution', 300)